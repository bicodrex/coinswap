...
fn main() -> Result<(), TakerError> {
    ...
    match args.command {
        ...
        Commands::WalletRestore { file}=> {
            ...
            let wallet_backup: WalletBackup = serde_json::from_str(&content).expect("Failed to deserialize wallet backup file");
            ...
            let restored_wallet = wallet_backup.restore(Path::new("./taker-wallet"), &rpc_config_test);
